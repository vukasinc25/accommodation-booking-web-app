...
func New(ctx context.Context, logger *log.Logger, uri string) (*UserRepo, error) {
	dburi := uri
...
func (pr *UserRepo) Ping() {
	...
	if err != nil {
		pr.logger.Println("Ping error: ", err)
	}

	// Print available databases
	databases, err := pr.cli.ListDatabaseNames(ctx, bson.M{})
	if err != nil {
		pr.logger.Println("Database names: ", err)
	}
...
func (ur *UserRepo) Insert(patient *User) error {
	...
	log.Println("Collection: ", patientsCollection)

	result, err := patientsCollection.InsertOne(ctx, &patient)
	if err != nil {
		log.Println("Error when tryed to insert into the database")
